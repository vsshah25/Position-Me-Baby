...
while(1)
    ...
    im=imcrop(im,[284.5 95.5 672 455]);
...
% [pos_cor_x pos_cor_y]
                flushinput(s1);
                RSSI_vect0(j,:)=fread(s1,9);
%                 char_data0=char(RSSI_vect0);
                
                flushinput(s1);
                RSSI_vect1(j,:)=fread(s1,9);
%                 char_data1=char(RSSI_vect1);
                
                flushinput(s1);
                RSSI_vect2(j,:)=fread(s1,9);
%                 char_data2=char(RSSI_vect2);
                
                flushinput(s1);
                RSSI_vect3(j,:)=fread(s1,9);
%                 char_data3=char(RSSI_vect3);
                
                flushinput(s1);
                RSSI_vect4(j,:)=fread(s1,9);
%                 char_data4=char(RSSI_vect4);
...
%                            total_data(j,:)=[Position Heading ID dt_cpu T char(RSSI_vect(j-1,1)) char(RSSI_vect(j-1,2)) char(RSSI_vect(j-1,3)) RSSI_vect(j-1
%                         if (fl_start==1),4) RSSI_vect(j-1,5) RSSI_vect(j-1,6) RSSI_vect(j-1,7) RSSI_vect(j-1,8) RSSI_vect(j-1,9)];
                            total_data(j,:)=[Position Heading ID dt_cpu T RSSI_vect0(j-1,2)-48 10*(RSSI_vect0(j-1,5)-48)+(RSSI_vect0(j-1,6)-48) RSSI_vect1(j-1,2)-48 10*(RSSI_vect1(j-1,5)-48)+(RSSI_vect1(j-1,6)-48) RSSI_vect2(j-1,2)-48 10*(RSSI_vect2(j-1,5)-48)+(RSSI_vect2(j-1,6)-48) RSSI_vect3(j-1,2)-48 10*(RSSI_vect3(j-1,5)-48)+(RSSI_vect3(j-1,6)-48) RSSI_vect4(j-1,2)-48 10*(RSSI_vect4(j-1,5)-48)+(RSSI_vect4(j-1,6)-48)];
%                         end
...
%                                                     plot(data_c(1,1),data_c(1,2),'g*')
    ...
    %     tic
      ...
      T_cpu=clock;
dt_cpu=etime(T_cpu,T_o);
T_o=T_cpu;
T_cpu=etime(T_cpu,T_init);
 T=T+dt_cpu;
    Timedisp=[T_cpu dt_cpu]
