...
for i=1:size(bot_id,1)
    id=bot_id(i);
        s_l=so{id};
for il=1:size(t_r{id},1)

s_l=so{id}+(t_r{id}(il)-t_r{id}(1))*K;
sl{id}=[sl{id} s_l];
end
xl{id}=A*cos(a*sl{id});
yl{id}=B*sin(b*sl{id});
...
id=bot_id(i);
 ...
 yl{id} = interp1(t_r{id},yl{id},tl);
 xl{id} = interp1(t_r{id},xl{id},tl);
Vl{id} = interp1(t_r{id},Vl{id},tl);
...
for j=1:size(tl',1)
     ...
     id=bot_id(i);
     plot(xl{id},yl{id},'k')
     axis([-A*1.1 A*1.1 -B*1.1 B*1.1])
     axis equal
     hold on
     str=[ num2str(id)];
     text(xa{id}(j)+2,ya{id}(j)+10,str); 
     patch(-xd+xa{id}(j),-yd+ya{id}(j),[0.85,0.85,1]);
     scatter(xa{id}(j),ya{id}(j),30,[0 0 0],'filled'); 
